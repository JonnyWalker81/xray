...
impl Search {
    ...
    fn update_match_variants(&self, match_variants: &Vec<MatchVariant>, name: &OsStr) -> Vec<MatchVariant> {
        let mut parent_path_len = self.parent_path.as_os_str().as_bytes().len();
        if parent_path_len > 0 {
            parent_path_len += 1;
        }

        let mut new_variants = Vec::<MatchVariant>::with_capacity(match_variants.len());
        ...
        for (name_index, character) in name.as_bytes().iter().map(|c| c.to_ascii_lowercase() as char).enumerate() {
            ...
            for variant in match_variants.iter() {
                ...
                if variant.query_index != previous_query_index {
                    previous_query_index = variant.query_index;
    ...
    fn test_search_subword_start_bonus() {
        ...
        match search.poll() {
            Ok(Async::Ready(Some(results))) => {
                assert_eq!(results[0].path, Path::new("cats/dogs/eagles"));
            },
